...
def LaCasce_rossby_rad(lat_0: float, lat_1:float, eigenvals:float) -> float:
    ...
    beta = 2*earth_angvel*np.cos(np.deg2rad(mean_lat))/6371e+03
...
if __name__ == "__main__":
    ...
    print(f"Rossby radii [km]: {R/1000}")
    print(f"1/sqrt(eigenvals) [km]: {(1/eigenvals)/1000}")
    lat_0 = config_parameters["set_domain"]["lat_min"]
    lat_1 = config_parameters["set_domain"]["lat_max"]
    print(f"LaCasce rossby radii [km]: {LaCasce_rossby_rad(lat_0, lat_1, eigenvals)/1000}")
