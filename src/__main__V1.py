...
@QGBaroclinic.subcommand("run")
class RunQGBaroclinic(cli.Application):
    def qgbaroclinic():
        """
        ...
        # SET OUTPUT FOLDER and SET LOGGING
        ncwrite = ncWrite(config.output.folder_path, filename=config.output.filename)
        log_path = ncwrite.logpath
        removed = False
